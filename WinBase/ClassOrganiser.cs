...
public class ClassOrganiser : KnowinGen
{
    ...
    public int Createstudentsuject(int _Standrd, int _Students, int Classsubject)
    {
        int _classid;

        string sql = "INSERT INTO tblstudent_indiviualsubject(ClassName,StudentId,SubjectId) VALUES (" + _Standrd + "," + _Students + "," + Classsubject + ")";
        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
        sql = "SELECT Id FROM tblclass where ClassName='" + _Standrd + "'";
        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
        if (m_MyReader.HasRows)
        {
            m_MyReader.Read();
    ...
    public int SurveyAnswer(int survey, string survey_ques, string Survey_answer,string studentid,string studentname)
	    {
	        int _classid;
	
	        string sql = "INSERT INTO tblSurveyAnswer(survey,survey_ques,Survey_answer,studentid,studentname) VALUES (" + survey + ",'" + survey_ques + "','" + Survey_answer + "','" + studentid + "','" + studentname + "')";
	        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
    ...
    public string GetClassMangSubMenuString(int _roleid, int _ModuleType)
    {
        ...
        _MenuStr = "<div class=\"list-group\"><a href=\"ClassDetails.aspx\" class=\"list-group-item\">Class Details</a>";
    ...
    public DataSet GetClsSubjectList(int _ClassID, int i_ExamId, int _BatchId)
    {
        DataSet Subjects = null;
    ...
    public double GetMinimumMarkOfSubject(int _ClassID, int i_ExamId, int _BatchId, int subjectID)
    {
        double MinMark = 0;
        string sql = "";
        sql = "SELECT tblclassexamsubmap.MinMark from tblclassexam inner join tblclassexamsubmap on tblclassexamsubmap.ClassExamId= tblclassexam.Id where tblclassexamsubmap.SubId=" + subjectID + " AND tblclassexam.ClassId=" + _ClassID + " AND tblclassexam.ExamId=" + i_ExamId + "";
        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
        if (m_MyReader.HasRows)
        {
            MinMark = double.Parse(m_MyReader.GetValue(0).ToString());
        }
        return MinMark;
