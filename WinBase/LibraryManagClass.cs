...
namespace WinBase
{
    public class LibraryManagClass : KnowinGen
    {
        ...
        public int AddBookMaster(string _Bookname, string _auther, string _Publisher, int _year, string _edition, int _type, int _catagory, string _count, double _Price, string Barcode, int Barcode_Type,string bookslno)
        {
            int _BookId = -1;
            string _Now = System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            string sql = " insert into tblbookmaster(BookName,Author,Publisher,Year,Edition,TypeId,CatagoryId,CreatedDate,Cost,Barcode,BarcodeType,Bookslno) values ('" + _Bookname + "','" + _auther + "','" + _Publisher + "'," + _year + ",'" + _edition + "'," + _type + "," + _catagory + ", '" + _Now + "'," + _Price + ",'" + Barcode + "','" + Barcode_Type + "','" + bookslno + "')";
            m_MyReader = m_MysqlDb.ExecuteQuery(sql);
        ...
        public int UpdateBooks(string _name, string _auther, string _Publisher, int year, string _Edition, int _Type, int _category, int _id, int _RackId, string _BookNo, double _Price,string Barcode,int Barcode_type,string bookslno)
        {
            int Id = -1;
            string sql = "update tblbookmaster set BookName='" + _name + "', Author='" + _auther + "', Publisher='" + _Publisher + "',Year=" + year + ", Edition='" + _Edition + "',TypeId=" + _Type + ",CatagoryId=" + _category + " , Cost=" + _Price + ",Bookslno='" + bookslno + "'  where Id=" + _id + "";
            m_MyReader = m_MysqlDb.ExecuteQuery(sql);
            Id = 1;
        ...
        public System.Data.DataSet GetBooksFromRackId(int RackId)
        {
            string sql = "select tblbooks.BookId as Id,tblbookmaster.BookName,tblbookmaster.Author,tblbookmaster.Publisher,tblbookmaster.Year,tblbookrack.RackName from tblbooks INNER JOIN tblbookmaster on tblbookmaster.Id = tblbooks.BookId INNER JOIN tblbookrack on tblbookrack.Id = tblbooks.RackId ";
            if (RackId != 0)
                sql = sql + " where tblbooks.RackId =" + RackId;
        ...
        public OdbcDataReader GetRack()
        {
            string sql = "select tblbookrack.Id, tblbookrack.RackName from tblbookrack ";
            return m_MysqlDb.ExecuteQuery(sql);
