...
namespace WinBase
{
    ...
    public class Lisencesing
    {
        ...
        public void UpdateLicense(string _lisencekey, out string _message)
        {
            ...
            string[] _temp = _licencetext.Split('$');
            string _LisenceMac = "", _SoftWare = "", _Version = "";
            if (1 <= _temp.Length) // checking if temp[index] exists or not
            {
                _LisenceMac = _temp[0];
            }
            if (4 <= _temp.Length)
            ...
            if (_SoftWare != "All" && _SoftWare!= m_MasterSoftWare)
            {
                _message = "Invalid License.";
            }
     ...
     public class AddressLisencesing
     {
         ...
         //custom //on 15/03/2018 for webmethod
         public string GetKeyAsChar()
         {
             string _SystemKey = "", C_SystemKey="";
             try
             {
                 _SystemKey = GetSystemKey();
                 C_SystemKey = _MyEncrption.Encrypt(_SystemKey);
             }
             catch (Exception ex)
             {
                 C_SystemKey = ex.Message;
             }
             return C_SystemKey;
         }
         public void RegisterLicenseNew(string _lisencekey, out string _msg)
         {
             string _licencetext = "";
             try
             {
                 _licencetext = _MyEncrption.Decrypt(_lisencekey);
                 string[] _temp = _licencetext.Split('$');
                 string _LisenceMac = "", _SoftWare = "", _Version = "";
                 if (1 <= _temp.Length) // checking if temp[index] exists or not
                 {
                 ...
                 if (!(_LisenceMac.Trim() == GetSystemKey().Trim()))
                 {
                     _msg = "Invalid License.";
