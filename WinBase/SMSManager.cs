...
# endregion
    ...
    public bool SaveStudentCredentials(string _PassWord, string _UserName, string _CanLogin, string _StudentName, string _StudentId)
    {
        bool _valid = false;
        try
        {
            ...
            string sql = "";
            string emailId = "";
            int MaxId = myGenobj.GetTableMaxId("tblstudentlogin", "Id");
         
            
                sql = "insert into tblstudentlogin(Id,Loginname,Pwd,Emailid,Studentname,Studentid,Status,CanLogin) values(" + MaxId + ",'" + _UserName + "','" + _PassWord + "','" + emailId + "' , '" + _StudentName + "' , " + _StudentId + "," + _CanLogin + ",0)";
                m_MysqlDb.ExecuteQuery(sql);              
                _valid = true;
    ...
    private bool MobNoNotexist(string _UserName, out string Parentid)
    {
        bool _valid = true;
        Parentid = "0";
        string sql = "select Id from tblstudentlogin where Loginname='" + _UserName + "'";
        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
        if (m_MyReader.HasRows)
        {
            Parentid = m_MyReader.GetValue(0).ToString();
    ...
    public bool UpdateStudentCredentials(string _PassWord, string _UserName, string _CanLogin,  string _ParentId)
    {
        bool _valid = false;
        try
        {

            string sql = "update tblstudentlogin set Loginname = '" + _UserName + "' ,Pwd = '" + _PassWord + "', Status =  " + _CanLogin + ",CanLogin = " + _CanLogin + " where Id=" + _ParentId;
            m_MysqlDb.ExecuteQuery(sql);
            _valid = true;
        }
        catch
    ...
    public void UpdateStudentLoginStatus(string _ParentId, string _CanLogin)
    {
        string sql = "update tblstudentlogin set CanLogin=" + _CanLogin + " where Studentid=" + _ParentId;
        m_MysqlDb.ExecuteQuery(sql);
    }
    ...
    public string GetStudPassword(string _ParentName, string _MobNo, string _StudentId)
    {
        KnowinEncryption MyEncription = new KnowinEncryption();
        string Password = "";
        string sql = "select Id,Pwd from tblstudentlogin where  Studentid='" + _StudentId + "'";
        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
        if (m_MyReader.HasRows)
        {
            Password = m_MyReader.GetValue(1).ToString();
    ...
    //public string GetPushDeviceList(string Audience, string SelectItem)
    //{
    //    string DeviceList = "",Separator=",",sql="";
    //    if (Audience == "ALL_STUDENTS")
    //    {
    //        sql = "SELECT DeviceUniqueId FROM tbldevice WHERE tbldevice.IsActive='1' AND tbldevice.DeviceType='PushDevice' ";
    //    }
    //    else if (Audience=="SELECTED_CLASS")
    //    {
    //        sql = "SELECT tbldevice.DeviceUniqueId FROM tblstudent inner join tbldevice on tbldevice.DeviceName = tblstudent.OfficePhNo WHERE tblstudent.LastClassId IN ("+SelectItem+");";
    //    }
    //    else if (Audience == "SELECTED_STUDENTS")
    //    {
    //        sql = "SELECT tbldevice.DeviceUniqueId FROM tblstudent inner join tbldevice on tbldevice.DeviceName = tblstudent.OfficePhNo WHERE tblstudent.Id IN (" + SelectItem + ");";
    //    }
    //    m_MyReader = m_MysqlDb.ExecuteQuery(sql);
    //    if (m_MyReader.HasRows)
    //    {
    //        while(m_MyReader.Read())
    //        {
    //            DeviceList = DeviceList + Separator + m_MyReader.GetValue(0).ToString();
    //        }
    //    }
    //    return DeviceList;
    //}

    public DataSet GetPushDeviceList(string Audience, string SelectItem)
    {
        string sql = "";
        if (Audience == "ALL_STUDENTS")
        {
            //sql = "SELECT DeviceUniqueId FROM tbldevice WHERE tbldevice.IsActive='1' AND tbldevice.DeviceType='PushDevice' ";
            sql = "SELECT tblstudent.Id,tblstudent.StudentName,tblstudent.GardianName,tbldevice.DeviceUniqueId,tbldevice.DeviceOs,tblstudent.AdmitionNo as AdmitionNo FROM tblstudent inner join tbldevice on tbldevice.DeviceName = tblstudent.OfficePhNo";
        }
        else if (Audience == "SELECTED_CLASS")//select with class id
        {
            //  sql = "SELECT tbldevice.DeviceUniqueId FROM tblstudent inner join tbldevice on tbldevice.DeviceName = tblstudent.OfficePhNo WHERE tblstudent.LastClassId IN ("+SelectItem+");";
            sql = "SELECT tblstudent.Id,tblstudent.StudentName,tblstudent.GardianName,tbldevice.DeviceUniqueId,tbldevice.DeviceOs,tblstudent.AdmitionNo as AdmitionNo FROM tblstudent inner join tbldevice on tbldevice.DeviceName = tblstudent.OfficePhNo WHERE tblstudent.LastClassId IN (" + SelectItem + ");";
        }
        else if (Audience == "SELECTED_STUDENTS")//select with student id
        {
            //sql = "SELECT tbldevice.DeviceUniqueId FROM tblstudent inner join tbldevice on tbldevice.DeviceName = tblstudent.OfficePhNo WHERE tblstudent.Id IN (" + SelectItem + ");";
            sql = "SELECT tblstudent.Id,tblstudent.StudentName,tblstudent.GardianName,tbldevice.DeviceUniqueId,tbldevice.DeviceOs,tblstudent.AdmitionNo as AdmitionNo FROM tblstudent inner join tbldevice on tbldevice.DeviceName = tblstudent.OfficePhNo WHERE tblstudent.Id IN (" + SelectItem + ");";
        }
        DataSet StudentData = m_MysqlDb.ExecuteQueryReturnDataSet(sql);
        return StudentData;
    }

    public string GetPushDeviceOS(string SelectItem)
    {
        string DeviceOS = "",sql = "";
        sql = "SELECT tbldevice.DeviceOS FROM tbldevice WHERE tbldevice.DeviceUniqueId = '" + SelectItem + "'";       
        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
        if (m_MyReader.HasRows)
        {
            while (m_MyReader.Read())
            {
                DeviceOS =  m_MyReader.GetValue(0).ToString();
            }
        }
        return DeviceOS;
    }
    public string GetPushDevicePhno(string SelectItem)
    {
        string DeviceOS = "", sql = "";
        sql = "SELECT tbldevice.DeviceName FROM tbldevice WHERE tbldevice.DeviceUniqueId = '" + SelectItem + "'";
        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
        if (m_MyReader.HasRows)
        {
            while (m_MyReader.Read())
