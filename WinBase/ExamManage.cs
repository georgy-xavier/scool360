...
public class ExamManage:KnowinGen
{
    ...
    public string GetFailExamMangMenuString(int _roleid)
    {
        CLogging logger = CLogging.GetLogObject();
        string _MenuStr;
        //if (m_SubExamMenuStr == "")
        //{
        _MenuStr = "<ul class=\"block\"><li><a href=\"FailedStudentReport.aspx\">Generate Report</a></li><li><a href=\"FailedStudMark.aspx\">Enter Marks</a></li>";
            logger.LogToFile("GetSubExamMangMenuString", "user is sending Request", 'I', CLogging.PriorityEnum.LEVEL_MEDIUM_IMPORTANT, LoginUserName);
            
            _MenuStr = _MenuStr + "</ul>";
            logger.LogToFile("GetSubExamMangMenuString", " Closing myreader  ", 'I', CLogging.PriorityEnum.LEVEL_DEBUG, LoginUserName);
            m_MyReader.Close();
            //m_SubExamMenuStr = _MenuStr;
    ...
    public void FillMarksFail(string _classid, string periodid, int _BatchId, int _Stud_id, int _Exam_id, string _FailedMarkColumn, Double _mark)
    {
        CLogging logger = CLogging.GetLogObject();
        int Scexam_id = -1;
        try
        {
            ...
            if (m_MyReader.HasRows)
            {
                ...
                if (m_MyReader.HasRows)
                {
                    logger.LogToFile("FillMarks", " Reading Success " + sql, 'I', CLogging.PriorityEnum.LEVEL_DEBUG, LoginUserName);
                    logger.LogToFile("FillMarks", "user is sending Request", 'I', CLogging.PriorityEnum.LEVEL_MEDIUM_IMPORTANT, LoginUserName);
                    sql = "UPDATE tblstudentmark   SET " + _FailedMarkColumn + "=" + _mark + " WHERE ExamSchId =" + Scexam_id + " AND Studid=" + _Stud_id;
                    logger.LogToFile("FillMarks", " Executing Query " + sql, 'I', CLogging.PriorityEnum.LEVEL_DEBUG, LoginUserName);
                    if (m_TransationDb == null)
                    {
                        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
                ...
                else
                {
                    logger.LogToFile("FillMarks", " Reading Failure " + sql, 'I', CLogging.PriorityEnum.LEVEL_DEBUG, LoginUserName);
                    logger.LogToFile("FillMarks", "user is sending Request", 'I', CLogging.PriorityEnum.LEVEL_MEDIUM_IMPORTANT, LoginUserName);
                    sql = "insert into tblstudentmark (ExamSchId,StudId," + _FailedMarkColumn + ") VALUES (" + Scexam_id + "," + _Stud_id + "," + _mark + ")";
                    logger.LogToFile("FillMarks", " Executing Query " + sql, 'I', CLogging.PriorityEnum.LEVEL_DEBUG, LoginUserName);
                    if (m_TransationDb == null)
                    {
                        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
    ...
    public void UpdateResult(int _studId, int _examscid, double _Max_Total, double _Total, int _SubCount, string _Result, DataSet _aggregatepassDs, int _scheduledid, int _Examid, int _classid, int _batchid, string _examperiod, int FailSubCount)
    {
        CLogging logger = CLogging.GetLogObject();
        double _Avg;
        string _StudGrade = "";
        ...
        if (_Max_Total==0)
        {
            ...
            _Avg = Math.Round(((_Total * 100) / _Max_Total), 2);
        }
        ...
        string sql = "UPDATE tblstudentmark   SET TotalMark=" + _Total + ",TotalMax=" + _Max_Total + ",Avg=" + _Avg + ",Grade='" + _StudGrade + "',Result='" + Currentresult + "' WHERE ExamSchId =" + _examscid + "  AND Studid=" + _studId + " AND FailSubCount=" + FailSubCount;
        logger.LogToFile("UpdateResult", " Executing Query " + sql, 'I', CLogging.PriorityEnum.LEVEL_DEBUG, LoginUserName);
        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
        //string sql1 = "update tblstudentmark set FailSubCount=" + FailSubCount + " WHERE ExamSchId =" + _examscid + "  AND Studid=" + _studId + "";
    ...
    public void UpdateResultFail(int _studId, int _examscid, double _Max_Total, double _Total, int _SubCount, string _Result, DataSet _aggregatepassDs, int _scheduledid, int _Examid, int _classid, int _batchid, string _examperiod)
    {
        CLogging logger = CLogging.GetLogObject();
        double _Avg;
        string _StudGrade = "";
        ...
        logger.LogToFile("UpdateResult", "user is sending Request", 'I', CLogging.PriorityEnum.LEVEL_MEDIUM_IMPORTANT, LoginUserName);
        string sql = "UPDATE tblstudentmark   SET TotalMark=" + _Total + ",TotalMax=" + _Max_Total + ",Avg=" + _Avg + ",Grade='" + _StudGrade + "',Result='" + Currentresult + "' WHERE ExamSchId =" + _examscid + "  AND Studid=" + _studId;
        logger.LogToFile("UpdateResult", " Executing Query " + sql, 'I', CLogging.PriorityEnum.LEVEL_DEBUG, LoginUserName);
        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
    ...
    public void ScheduleSubjects(int _ExSchId, string _columnname,string _failedcolumnname, int _subjectId, int _timeid, DateTime _examdate, int SubOrder)
    {
        CLogging logger = CLogging.GetLogObject();        
        logger.LogToFile("ScheduleSubjects", "user is sending Request", 'I', CLogging.PriorityEnum.LEVEL_MEDIUM_IMPORTANT, LoginUserName);
        string sql = "INSERT INTO tblexammark(ExamSchId,MarkColumn,FailedMarkColumn,SubjectId,TimeSlotId,ExamDate,SubjectOrder) VALUES (" + _ExSchId + ",'" + _columnname + "','"+_failedcolumnname+"'," + _subjectId + "," + _timeid + ", '" + _examdate.Date.ToString("s") + "'," + SubOrder + ")";
        logger.LogToFile("ScheduleSubjects", " Executing Query " + sql, 'I', CLogging.PriorityEnum.LEVEL_DEBUG, LoginUserName);
        m_MyReader = m_MysqlDb.ExecuteQuery(sql);
        logger.LogToFile("ScheduleSubjects", " Closing myreader  " + sql, 'I', CLogging.PriorityEnum.LEVEL_DEBUG, LoginUserName);
        m_MyReader.Close();
    ...
    private DataSet GetSubjects(int _ClassId, int _ExamId)
    {
       ...
       // string sql = "select tblsubjects.Id , tblsubjects.subject_name from tblclasssubmap inner join  tblsubjects on tblsubjects.Id = tblclasssubmap.SubjectId where tblclasssubmap.ClassId="+_ClassId;
        string sql = "select distinct tblsubjects.Id , tblsubjects.subject_name from tblclassexamsubmap INNER join   tblsubjects on tblsubjects.Id = tblclassexamsubmap.SubId inner join tblclassexam on tblclassexam.Id =  tblclassexamsubmap.ClassExamId where tblclassexam.ClassId=" + _ClassId + " and tblclassexam.ExamId =" + _ExamId + "";
        MyDataSet = m_MysqlDb.ExecuteQueryReturnDataSet(sql);
        return MyDataSet;
    }
    ...
    public void UpdateInexammarkcolumn(int classexamId, int _SubjectId, int BatchId)
    {
        ...
        if (Dt != null && Dt.Tables != null && Dt.Tables[0].Rows.Count > 0)
        {
            ...
            SubOrder = SubOrder + 1;
            ScheduleSubjects(_ExSchId, _columnname,failedmarkcol, _SubjectId, _timeid, _examdate, SubOrder);
