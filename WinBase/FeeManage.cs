...
public class FeeManage : KnowinGen
{
    ...
    public string GenBillWithTax(double _Total, int StudentId, string _PayMode, string _payId, string _bank, string _Date, int _UserId, int _BatchId, string _ClassId, string _StudentName, int _Regular, string _TempId, string _OtherReferance, string _BillTable,int _TaxId,double _TaxAmnt)
    {
        string _billno = "0";
        if (m_BillPrefix == null)
        {
        ...
        if (m_TransationDb != null)
        {
            ...
            if (_TempId != "")//Setting student id to zero for registered students
                StudentId = 0;
            string sql = "insert into " + _BillTable + " (Id,StudentID,TotalAmount,Date,PaymentMode,PaymentModeId,BankName,UserId,CreatedDateTime,Counter,AccYear,ClassID,StudentName,BillNo,RegularFee,TempId,OtherReference,TaxId,TaxAmnt) values(" + _FeeId + "," + StudentId + "," + _Total + ",'" + _BillDate.ToString("s") + "','" + _PayMode + "','" + _payId + "','" + _bank + "'," + _UserId + ",'" + _CreatedDate.ToString("s") + "'," + _FeeMaxId + "," + _BatchId + "," + _ClassId + ",'" + _StudentName + "','" + _billno + "'," + _Regular + ",'" + _TempId + "','" + _OtherReferance + "','" + _TaxId + "','" + _TaxAmnt + "')";
            m_TransationDb.TransExecuteQuery(sql);
    ...
    public bool TaxEnabled()
    {
        ...
        if (m_MyReader.HasRows)
        {
            int val = int.Parse(m_MyReader.GetValue(0).ToString());
            if (val == 1)
            {
                valid = true;
            }
