...
public class StudentManagerClass:KnowinGen
{
    ...
    public string GetSubStudentMangMenuString(int _roleid,int _Type)
    {
        ...
        {
            _MenuStr = "<div class=\"list-group\"><a href=\"StudentDetails.aspx\" class=\"list-group-item\">Student Details</a>";
            ...
            if (m_MyReader.HasRows)
            {
                while (m_MyReader.Read())
                {
                    _MenuStr = _MenuStr + "<a class=\"list-group-item\" href=\"" + m_MyReader.GetValue(1).ToString() + "\">" + m_MyReader.GetValue(0).ToString() + "</a>";
                }
            }
            _MenuStr = _MenuStr + "</div>";
            m_MyReader.Close();
    ...
    public string GetSubStudentMangerMenuString(int _roleid, int _Type)
    {
        string _Field = "Live";
        if (_Type == 1)
            _Field = "Live";
        ...
        {
            _MenuStr = "<div class=\"list-group\"><a href=\"StudentDetails.aspx\" class=\"list-group-item\">Student Details</a>";
            string sql = "SELECT DISTINCT tblaction.MenuName, tblaction.Link FROM tblaction INNER JOIN  tblroleactionmap ON tblaction.Id = tblroleactionmap.ActionId INNER JOIN tblstudentsubmenu on tblstudentsubmenu.ActionId = tblaction.Id and tblstudentsubmenu.Student=1 WHERE  tblroleactionmap.RoleId=" + _roleid + " AND ((tblroleactionmap.ModuleId=2 AND tblaction.ActionType='SubLink') or tblaction.ActionType='StudSubLink') order by tblaction.`Order` ";
            m_MyReader = m_MysqlDb.ExecuteQuery(sql);
            if (m_MyReader.HasRows)
            {
                while (m_MyReader.Read())
    ...
    public int CreateStudent(string _studname, string _sex, DateTime _Dob, string _Fathername, string _address, int _joiningbatchid, int _joiningStandard, DateTime _Joindate, int _StandardId, int _classid, string _admisionNo, int _Bloodgroupid, string _nationality, int mothertoungid, string _mothername, string _Fathedu, string _mothedu, string _fatherOcc, string _motherOcc, double _Anualincom, string _addrspresent, string _location, string _State, int _pin, string _resedphon, string _officephon, string _email, int nofbrother, int nofsis, int firstlng, int studcatogory, int _relegionId, int CastId, int _CurrentBatchId, int _AdmissionType, string _TempStudId, int _UseBus, int _UseHostel, string StudentId, string aadharno)
    {
        ...
        try
        {
            ...
            if (_studentid != -1)
            {
                ...
                {
                    m_DbLog.LogToDbNoti(m_UserName, "Created Student", "Student Named " + _studname + " is created and approved", 1,1, m_TransationDb);
    ...
    public string ToStripString(int _studid,string _imgurl,int _StudType)
    {
        ...
        string _studentname="", _Admissionno="", _class="", _age="", _rollno="Not Assigned",_StudentId="",_contactNo="";
        int Year;
        DateTime Today = DateTime.Now.Date;
        string Sql = "select tblview_student.StudentName, tblview_student.AdmitionNo, tblview_student.DOB,tblclass.ClassName,tblview_student.RollNo,tblview_student.StudentId,tblview_student.OfficePhNo from tblview_student INNER join tblclass on tblview_student.ClassId=tblclass.Id where tblview_student.Id=" + _studid;
        ...
        if (m_MyReader.HasRows)
        {
            ...
            _contactNo = m_MyReader.GetValue(6).ToString();
            m_MyReader.Close();
        }
        ...
        StringBuilder _pupilTopData = new StringBuilder("<div id=\"winschoolStudentStrip\"><table class=\"NewStudentStrip\" width=\"100%\"><tr><td class=\"left1\"></td><td class=\"middle1\" ><table><tr><td><img alt=\"\" src=\"" + _imgurl + "\" width=\"75px\" height=\"80px\" /></td><td> </td><td><table width=\"500\"><tr><td class=\"attributeValue\">Name</td><td></td><td>:</td><td></td><td class=\"DBvalue\">" + _studentname + "</td><td class=\"attributeValue\">Contact No</td><td></td><td>:</td> <td></td><td class=\"DBvalue\">" + _contactNo + "</td></tr><tr><td class=\"attributeValue\">Final Class</td><td></td><td>:</td><td></td><td class=\"DBvalue\">" + _class + "</td><td class=\"attributeValue\">" + _AdmissionStr_Left + "</td><td></td><td>:</td> <td></td> <td class=\"DBvalue\"> " + _AdmissionStr_Right + "</td> <td></td> </tr><tr><td class=\"attributeValue\">Roll No</td> <td></td><td>:</td> <td></td> <td class=\"DBvalue\">" + _rollno + "</td> <td class=\"attributeValue\">Age</td> <td></td><td>:</td> <td></td><td class=\"DBvalue\">" + _age + "</td></tr> </table></td>  </tr></table>  </td>  <td class=\"right1\"> </td> </tr></table></div>");               
   ...
   public void UpdateStudentOtherDetails(int _Bloodgroupid, string _nationality, int mothertoungid, string _mothername, string _Fathedu, string _mothedu, string _fatherOcc, double _Anualincom, string _addrspresent, string _location, string _State, int _pin, string _resedphon, string _officephon, string _email, int nofbrother, int nofsis, int firstlng, int studcatogory, int _studentId, int _AdmissionType, int _UseBus, int _UseHostel, string _motherOcc, string _aadharno)
    ...
    private StringBuilder LoadStudentDetails(int StudId, StringBuilder _TC)
    {
        ...
        if (MyReader.HasRows)
        {
            ...
            _TC.Replace("[$StudName$]", textInfo.ToTitleCase(MyReader.GetValue(3).ToString().ToLower()));
            _TC.Replace("[$SchoolName$]", textInfo.ToTitleCase(MyReader.GetValue(4).ToString().ToLower()));
            _TC.Replace("[$AdmissionNo$]", MyReader.GetValue(5).ToString());
            _TC.Replace("[$CumulativeNo$]", MyReader.GetValue(6).ToString());
            _TC.Replace("[$Sex$]", textInfo.ToTitleCase(MyReader.GetValue(7).ToString().ToLower()));
            _TC.Replace("[$FatherName$]", textInfo.ToTitleCase(MyReader.GetValue(8).ToString().ToLower()));
            _TC.Replace("[$MotherName$]", textInfo.ToTitleCase(MyReader.GetValue(33).ToString().ToLower()));
            string DOB = "";
            string DOBinWords = "";
            ...
            _TC.Replace("[$DOB$]", DOB);
            _TC.Replace("[$DOBinword$]", textInfo.ToTitleCase(DOBinWords.ToLower()));
            _TC.Replace("[$ResAddress$]", textInfo.ToTitleCase(MyReader.GetValue(34).ToString().ToLower()));
            _TC.Replace("[$Nationality$]", textInfo.ToTitleCase(MyReader.GetValue(9).ToString().ToLower()));
            _TC.Replace("[$Religion$]", textInfo.ToTitleCase(MyReader.GetValue(10).ToString().ToLower()));
            _TC.Replace("[$Caste$]", textInfo.ToTitleCase(MyReader.GetValue(11).ToString().ToLower()));
            _TC.Replace("[$SCST$]", textInfo.ToTitleCase(MyReader.GetValue(12).ToString().ToLower()));
            _TC.Replace("[$LastStandard$]", MyReader.GetValue(14).ToString());
            _TC.Replace("[$LanguageStudied$]", textInfo.ToTitleCase(MyReader.GetValue(15).ToString().ToLower()));
            _TC.Replace("[$SubjectStudied$]", textInfo.ToTitleCase(MyReader.GetValue(36).ToString().ToLower()));
            _TC.Replace("[$Medium$]", textInfo.ToTitleCase(MyReader.GetValue(16).ToString().ToLower()));
            ...
            _TC.Replace("[$Result$]", textInfo.ToTitleCase(MyReader.GetValue(19).ToString().ToLower()));
            _TC.Replace("[$Fee$]", textInfo.ToTitleCase(MyReader.GetValue(20).ToString().ToLower()));
            _TC.Replace("[$FeeCon$]", textInfo.ToTitleCase(MyReader.GetValue(21).ToString().ToLower()));
            _TC.Replace("[$Scholarship$]", textInfo.ToTitleCase(MyReader.GetValue(22).ToString().ToLower()));
            _TC.Replace("[$Medical$]", textInfo.ToTitleCase(MyReader.GetValue(23).ToString().ToLower()));
            ...
            _TC.Replace("[$LastExamDetails$]", textInfo.ToTitleCase(MyReader.GetValue(37).ToString().ToLower()));
            _TC.Replace("[$Character$]", textInfo.ToTitleCase(MyReader.GetValue(29).ToString().ToLower()));
             ...
             if (DateTime.TryParse(MyReader.GetValue(39).ToString(), out Date_LastClassDateIssue))
            {
             ...
             _TC.Replace("[$Enrollmentno$]", MyReader.GetValue(40).ToString());
