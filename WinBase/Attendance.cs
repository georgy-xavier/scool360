...
//
        ...
        public int GetNumberOfworkingDayForStaff()
        {
            int _TotalworkingDay = 0;
        ...
        public bool StaffAttendanceReport(int _staffId,string _startDate, string _endDate, out string _msg, out string[] Dates, out string[] AttData)
        {
            Dates = new string[40];
            AttData = new string[40];
            _msg = "";
            ...
            try
            {
                ...
                do
                {
                    ...
                    _msg = "";
                    // Date is holiday or not
                    if (!dateisHoliday(0, int.Parse(dateId[j].ToString())) && !IsDefaultHoliday(_GenObj.GetDateFromText(Dates[j].ToString()).Date.DayOfWeek))
                    {


                        
                        if (IsstaffReportedOnSelectedDate(_staffId, _GenObj.GetDateFromText(Dates[j].ToString()), out Status))
                        {
        ...
        public bool IsstaffReportedOnSelectedDate(int _staffId, DateTime Day, out int Status)
        {
            Status = -1;
            bool exist = false;
            string sql = "SELECT tblstaffattendance.MarkStatus from tblstaffattendance where tblstaffattendance.MarkDate='" + Day.Date.ToString("s") + "' and tblstaffattendance.StaffId=" +_staffId + "";
            m_MyReader = m_MysqlDb.ExecuteQuery(sql);
            if (m_MyReader.HasRows)
            {
                int.TryParse(m_MyReader.GetValue(0).ToString(), out Status);
