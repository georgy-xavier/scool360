...
public class StaffManager:KnowinGen
{
    ...
    public int CreateStaff(string _StaffName, string _password, string _JoiningDate, string _Address, string _Sex, float _Experience, string _Desig, string _PhNo, string _Email, string _StaffId, string _EduQuli, int _Login, int _Role, String _Dob, string _ParentGrp, out string _Message)
    {
        CLogging logger = CLogging.GetLogObject();
        ...
        try
        {
           ...
           // m_MyReader = m_MysqlDb.ExecuteQuery(sql);
            ...
            if (_UserId != -1)
            {
                sql = "INSERT INTO tblstaffdetails(UserId,JoiningDate,Address,Sex,Experience,Designation,PhoneNumber,EduQualifications,Dob) VALUES ( " + _UserId + ", '" + _joingDate.Date.ToString("s") + "', '" + _Address + "', '" + _Sex + "', " + _Experience + ", '" + _Desig + "', '" + _PhNo + "', '" + _EduQuli + "','" + _DateOfBirth.Date.ToString("s") + "')";
                if (m_TransationDb != null)
                {
                    m_TransationDb.ExecuteQuery(sql);
                }
    ...
    public int UpdateStaff(string StaffName, string Password, string JoiningDate, string Address, string Sex, float Experience, string Desig, string PhNo, string Email, string StaffId, string EduQuli, int Login, int Role, string Dob, int StfId, int _SelectedGroupId)
    {
        CLogging logger = CLogging.GetLogObject();
        ...
        try
        {
           ...
           // m_MyReader = m_MysqlDb.ExecuteQuery(sql);
            ...
            if (StfId != -1)
            {
                sql = "UPDATE tblstaffdetails SET UserId= '" + StfId + "',JoiningDate= '" + _joingDate.Date.ToString("s") + "',Address= '" + Address + "',Sex= '" + Sex + "',Experience= '" + Experience + "',Designation= '" + Desig + "',PhoneNumber= '" + PhNo + "',EduQualifications= '" + EduQuli + "',Dob= '" + _DateOfBirth.Date.ToString("s") + "' WHERE UserId=" + StfId;
                MysqlTranDb.TransExecuteQuery(sql);
