...
public partial class StudentPerform : System.Web.UI.Page
{
   ...
   // private ExamManage MyExamMang;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (CheckExamConfiguration())
            {
                ...
                //Session["StudType"] = int.Parse(Session["StudId"].ToString());
    ...
    {
        string sql = "select tblstudentmark.ExamSchId, tblexammaster.ExamName , tblperiod.Period from tblexamschedule inner join tblclassexam on tblexamschedule.ClassExamId= tblclassexam.Id inner join tblexammaster ON tblexammaster.Id= tblclassexam.ExamId inner join tblperiod on tblperiod.Id = tblexamschedule.PeriodId inner join tblstudentmark on tblstudentmark.ExamSchId= tblexamschedule.Id where tblexamschedule.BatchId="+MyUser.CurrentBatchId+" AND tblstudentmark.StudId="+Session["StudId"].ToString()+" AND tblexammaster.`Status`=1";
    ...
    private void LoadExamToDrpList()
    {
        ...
        string sql = "select distinct tblexammaster.Id, tblexammaster.ExamName from tblexamschedule inner join tblclassexam on tblexamschedule.ClassExamId= tblclassexam.Id inner join tblexammaster ON tblexammaster.Id= tblclassexam.ExamId inner join tblperiod on tblperiod.Id = tblexamschedule.PeriodId inner join tblstudentmark on tblstudentmark.ExamSchId= tblexamschedule.Id where tblexamschedule.BatchId=" + MyUser.CurrentBatchId + " AND tblstudentmark.StudId=" + Session["StudId"].ToString() + " AND tblexammaster.`Status`=1 and tblexammaster.ExamTypeId=" + int.Parse(Drp_ExamType.SelectedValue);
        MyReader = MyStudMang.m_MysqlDb.ExecuteQuery(sql);
        if (MyReader.HasRows)
        {
            Btn_Report.Enabled = true;
    ...
    protected void Btn_Report_Click(object sender, EventArgs e)
    {
        ...
        DataSet MyExamData = GetExamData(int.Parse(Session["StudId"].ToString()), i_ExamId, MyUser.CurrentBatchId);
