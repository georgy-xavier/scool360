...
namespace WC.WinerSchool.DL
{
    public class DataManager
    {
        ...
        public SchoolFeeCollection GetAllSchoolTotalFee(FeeCollectionFilter filter)
        {
            List<School_Details> schoolList = new List<School_Details>();
            int totalSchools = 0;
            try
            {
                using (centraldbEntities cdb = new centraldbEntities())
                {
                    ...
                    var allSchools = (from st in cdb.tblschool_list
                                where st.IsActice == 1
                                select new { st.Id, st.SchoolName, st.HostName, st.DatabaseName, st.UserName, st.Password, st.FilePath }).ToList();
                    totalSchools = allSchools.Count();
                    var list = allSchools.Skip((filter.page-1) * filter.pageSize).Take(filter.pageSize).ToList();
                    if (list != null)
                    {
                        foreach (var school in list)
                        {
                            if (school != null)
                            {
                                strADConnection = "Server=" + school.HostName + ";Database=" + school.DatabaseName + ";Uid=" + school.UserName + ";Pwd=" + school.Password + ";";
                                var data = GetSchoolTotalFee(strADConnection, filter);
                                if (data != null)
                                {
            ...
            {
                schoolList = new List<School_Details>();
            }


            return new SchoolFeeCollection { TotalSchoolCount = totalSchools, SchoolList = schoolList };
        }

        public List<School_Details> GetAllSchoolStrength()
        {
            List<School_Details> schoolList = new List<School_Details>();
            try
            {
                using (centraldbEntities cdb = new centraldbEntities())
                {
                    var list = (from st in cdb.tblschool_list
                                    where st.IsActice == 1
                                    select new { st.Id, st.SchoolName, st.HostName, st.DatabaseName, st.UserName, st.Password, st.FilePath }).ToList();
                    ...
                    if (list != null)
                    {
                        foreach (var school in list)
                        {
                            if (school != null)
                            {
                                ...
                                ecb.Metadata = "res://*/WinerModel.csdl|res://*/WinerModel.ssdl|res://*/WinerModel.msl";
                                ecb.Provider = "MySql.Data.MySqlClient";
                                ecb.ProviderConnectionString = "Server=" + school.HostName + ";Database=" + school.DatabaseName + ";Uid=" + school.UserName + ";Pwd=" + school.Password + ";";
                                FilePath = school.FilePath;

                                strETConnection = ecb.ConnectionString;
                                var data = GetSchoolStrength(strETConnection);
                                if (data != null)
                                {
                                    data.SchoolID = school.Id;
                                    data.SchoolName = school.SchoolName;
        ...
        public List<UserDetails> GetAllSchoolStaffStrength()
        {
                    ...
                    if (list != null)
                    {
                        foreach (var school in list)
                        {
                            if (school != null)
                            {
                                ...
                                strETConnection = ecb.ConnectionString;
                                var data = GetStaffsStrength(strETConnection);
                                if (data != null)
                                {
                                    data.SchoolID = school.Id;
                                    data.SchoolName = school.SchoolName;
        ...
        private School_Details GetSchoolTotalFee(string adoConnection, FeeCollectionFilter filter)
        {
            School_Details school = null;
            try
            {
                string subsql = "";
                if(filter.FromDate!= null)
                {
                    subsql = " and tblview_feebill.Date >='" + filter.FromDate.Value.Date.ToString("s") + "'";
                }
                if(filter.ToDate!= null)
                {
                    subsql = subsql + " and tblview_feebill.Date <='" + filter.ToDate.Value.Date.ToString("s") + "'";
                   
                }

                string sql = "select SUM(TotalAmount) from tblview_feebill where tblview_feebill.status=1 and tblview_feebill.Canceled=0" +subsql;
                using (MySqlConnection objConnection = new MySqlConnection(adoConnection))
                {
                    objConnection.Open();
                    using (MySqlCommand objCommand = new MySqlCommand())
                    {
                        ...
                        using (MySqlDataReader objReader = objCommand.ExecuteReader())
                        {
                            if (objReader.HasRows)
                                if (objReader.Read())
                                {
                                    decimal.TryParse(objReader.GetValue(0).ToString(), out totalFees);
                                }
                        }

                        school = new School_Details();
                        school.TotalFees = totalFees;
                    }
                }

                sql = "SELECT SUM(tblfeestudent.BalanceAmount) FROM tblfeestudent inner join tblstudent ON  tblstudent.Id= tblfeestudent.StudId where tblstudent.Status=1 AND tblfeestudent.Status<>'Paid' and tblfeestudent.`Status`<> 'Fee Exemtion'";
                using (MySqlConnection objConnection = new MySqlConnection(adoConnection))
                {
                    objConnection.Open();
                    using (MySqlCommand objCommand = new MySqlCommand())
        ...
        private School_Details GetSchoolStrength(string strETConnection)
        {
            School_Details school = null;
            try
            {
                using (WinErdbEntities db = new WinErdbEntities(strETConnection))
                {
                    var students = (from st in db.tblstudents
                                 where st.Status == 1
                                 select new { st.Id, st.StudentName, st.Sex}).ToList();

                    if (students != null)
                    {
                        school = new School_Details();
                        school.TotalStudents = students.Count();
                        school.TotalMale = students.Where(s => s.Sex.Equals("Male")).Count();
                        school.TotalFemale = students.Where(s => s.Sex.Equals("Female")).Count();
                    }
                }
            }
            catch
        ...
        public UserDetails GetStaffsStrength(string strETConnection)
        {
            UserDetails school = null;
            try
            {
                using (WinErdbEntities db = new WinErdbEntities(strETConnection))
                {
                    var staffs = (from st in db.tblusers
                                    where st.Status == 1
                                    select new { st.Id, st.SurName}).ToList();

                    if (staffs != null)
                    {
                        school = new UserDetails();
                        school.TotalStaffs = staffs.Count();
                        //school.TotalMaleStaffs = staffs.Where(s => s.Sex.Equals("Male")).Count();
                        //school.TotalFemaleStaffs = staffs.Where(s => s.Sex.Equals("Female")).Count();
                    }
                }
            }
            catch
