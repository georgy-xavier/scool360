...
namespace WinErParentLogin
{
    public class Pdf
    {
        ...
        internal bool CreateFeeReciptPdf(string _BillNo, string _BatchName, int _BatchId, string _physicalpath, out string _PdfName, out string _ErrorMsg, int _BillTypeValue)
        {
            _PdfName = "";
            _ErrorMsg = "";
        ...
        internal bool CreateFeeRecipt(string _BillNo, string _physicalpath, out string _PdfName, out string _ErrorMsg, int _Value,int _count)
        {
            ...
            try
            {
                ...
                GetStudentDetails(_BillNo, out StudentName, out AdmissionNo, out RollNo, out ClassName);

                string sql = "select distinct Date_Format(tbltransactionclearence.PaidDate,'%d-%m-%y') as PaidDate from tbltransactionclearence where tbltransactionclearence.BillNo='" + _BillNo + "'";
                ...
                sql = "select tbltransactionclearence.TransationId from tbltransactionclearence where tbltransactionclearence.BillNo='" + _BillNo + "'";
                ...
                sql2 = "select tbltransactionclearence.Amount as Total ,date_format(tbltransactionclearence.Date,'%d-%m-%Y') AS 'PaidDate' from tbltransactionclearence where tbltransactionclearence.BillNo='" + _BillNo + "'";

                MyReader = m_MysqlDb.ExecuteQuery(sql2);
                if (MyReader.HasRows)
                {
        ...
        {
            // loade fee details
            iTextSharp.text.Table Tbl_FeeDetails = FR_LoadFeeDetailsOnFullPage(ref DocFR, writerFR, _BillNo, _StudentId, _StudentType, _BillType, _CollegeId, _TotalAmount, _CashAmt, _ChequeAmt, _DDAmt, _ChequeNo, _ChequeBankName, _DDNo, _DDBankName, _physicalpath, _CreatedUser, _CreatedDate);
        ...
        private iTextSharp.text.Table FR_LoadFeeDetailsOnFullPage(ref Document DocFR, PdfWriter writerFR, string _BillNo, int _StudentId, int _StudentType, int _BillType, int _CollegeId, double _TotalAmount, double _CashAmt, double _ChequeAmt, double _DDAmt, string _ChequeNo, string _ChequeBankName, string _DDNo, string _DDBankName, string _physicalpath, string _CreatedUser, DateTime _CreatedDate)
        {
            ...
            sql = "SELECT tblfeeaccount.AccountName,tblview_transaction.Amount, tblview_transaction.TransType, tblview_transaction.AccountTo, tblfeeschedule.Id,tblview_transaction.BalanceAmount FROM tblview_transaction inner JOIN tblfeeschedule on tblview_transaction.FeeId= tblfeeschedule.Id inner join tblfeeaccount on tblfeeschedule.FeeId= tblfeeaccount.Id where tblview_transaction.BillNo='" + _BillNo + "' and tblview_transaction.UserId=" + _StudentId + " and tblview_transaction.AccountTo !=3";
            ...
            sql = "SELECT tblfeeaccount.AccountName,tblview_transaction.Amount, tblview_transaction.TransType, tblview_transaction.AccountTo, tblfeeschedule.Id,tblview_transaction.BalanceAmount  FROM tblview_transaction inner JOIN tblfeeschedule on tblview_transaction.FeeId= tblfeeschedule.Id inner join tblfeeaccount on tblfeeschedule.FeeId= tblfeeaccount.Id where tblview_transaction.BillNo='" + _BillNo + "' and tblview_transaction.UserId=" + _StudentId;
            ...
            if (_total_item_count > 0)
            {
                ...
                for (int k = 0; k < m_MyDataSet.Tables[0].Rows.Count; k++)
                {
                    ...
                    _BalanceAmount = double.Parse(m_MyDataSet.Tables[0].Rows[k][5].ToString());
                    _Discount = 0;
        ...
        private iTextSharp.text.Table FR_LoadBillAndStudentInfoDetilsToTable(string _BillNo, DateTime _BillDate, int _StudentId, string _BatchName, int _ClassId)
        {
            ...
            sql = "select tblview_student.StudentName, tblview_student.AdmitionNo, tblclass.ClassName from tblview_student inner join tblclass on   tblclass.Id=" + _ClassId + " AND tblview_student.Id=" + _StudentId;
        ...
        {// initializing the value
            ...
            //sql = "select tblfeebill.Id, tblfeebill.StudentId,tblfeebill.TotalAmount, tblfeebill.UserId, tblview_user.UserName,tblfeebill.`Date`, tblstudentclassmap.ClassId,tblfeebill.CreatedDateTime from tblfeebill INNER JOIN tblview_user on tblfeebill.UserId= tblview_user.Id inner join tblstudentclassmap on tblstudentclassmap.BatchId=" + _BatchId + " where tblfeebill.BillNo='" + _BillNo + "'";
            sql = "select tblview_feebill.TransationId, tblview_feebill.StudentId,tblview_feebill.TotalAmount, tblview_feebill.CollectedUser, tblview_user.UserName,tblview_feebill.`Date`, tblview_studentclassmap.ClassId,tblview_feebill.`Date` from tblview_feebill INNER JOIN tblview_user on tblview_feebill.CollectedUser= tblview_user.Id inner join tblview_studentclassmap on tblview_studentclassmap.StudentId=tblview_feebill.StudentId where tblview_feebill.BillNo='" + _BillNo + "'";
            m_MyReader = m_MysqlDb.ExecuteQuery(sql);
            if (m_MyReader.HasRows)
            {
                ...
                _CreatedDate = DateTime.Parse(m_MyReader.GetValue(7).ToString());
                ...
                //string sql1="SELECT  tblfeebill.PaymentMode, tblfeebill.TotalAmount, tblfeebill.PaymentModeId, tblfeebill.BankName FROM tblfeebill where tblfeebill.BillNo='"+_BillNo+"'";
                string sql1 = "SELECT  tblview_feebill.PaymentMode, tblview_feebill.TotalAmount, tblview_feebill.PaymentModeId, tblview_feebill.BankName FROM tblview_feebill where tblview_feebill.BillNo='" + _BillNo + "'";
        ...
        private iTextSharp.text.Table FR_LoadSignAndTotalDetails(iTextSharp.text.Table Tbl_SignAndTotal, double _TotalAmount, double _CashAmt, double _ChequeAmt, double _DDAmt, string _ChequeNo, string _ChequeBankName, string _DDNo, string _DDBankName, string _CreatedUser, DateTime _CreatedDate)
        {
            ...
            Tbl_SignAndTotal.DefaultCell.BorderWidthLeft = 1;
            Tbl_SignAndTotal.AddCell(new Phrase(" Bill Issued By: " + _CreatedUser + "         Bill Date: " + _CreatedDate, MyFonts[3]));
